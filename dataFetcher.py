			...
			if response == 200 and website != None:
				...
				if website == 'kissmanga':
					...
					for i in rawChapters:
						try:
							chapters[i.a.text.strip()] = 'https://kissmanga.org'+i.a.attrs['href'] 
						except:
							pass
					chapters = dict(reversed(list(chapters.items())))

					return(name.text.strip() , desc.text.strip(), chapters , 'https://kissmanga.org'+cover.img.attrs['src'] )
				if website == 'readm.org':
					...
					chapters = {}
					for i in rawChapters:
						chapters[i.h6.text.strip()] =  'https://readm.org'+str(i.a.attrs['href'])
					chapters = dict(reversed(list(chapters.items())))
				...
				if website == 'mangakakalot':
					...
					chapters = {}
					for i in rawChapters:
						href = str(i.span.a).split('href=')[-1].split('title=')[0].strip('"')
